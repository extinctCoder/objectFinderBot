...
def filter_colour(frame):
    ...
    filter_one = cv2.inRange(bgr_hsv, np.array(
        [160, 160, 10]), np.array([190, 255, 255]))
    bgr_crcb = cv2.cvtColor(frame, cv2.COLOR_BGR2YCrCb)
    filter_two = cv2.inRange(bgr_crcb, np.array(
        (0., 165., 0.)), np.array((255., 255., 255.)))
...
def track_ball(filter):
    ...
    temp_contours, _ = cv2.findContours(
        filter, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
...
def draw_frame(filter, frame):
    cnts = imutils.grab_contours(cv2.findContours(
        filter.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE))
    center = None
    if len(cnts) > 0:
